...
export const FormikTextInput: React.FC<TextInputProps> = (props) => {
  ...
  return (
    <TextInput id={field.name} {...field} {...props} errorText={meta.touched ? meta.error : undefined} />
  );
};
```
...
export const FormikDropdown: React.FC<DropdownProps> = (props) => {
  const [field, meta] = useField(props.name);
  return (
    <Dropdown id={field.name} {...field} {...props} errorText={meta.touched ? meta.error : undefined} />
  );
};
```
